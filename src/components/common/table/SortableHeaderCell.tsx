...
export default function SortableHeaderCell({
    ...
    let sortOrder = useGetSearchParamValue(
        _paramKeys.sortOrder,
        defaultSortOrder,
    );
    let sortColumn = useGetSearchParamValue(
    ...
    return (
        ...
        >
            <Label className={cn("cursor-pointer inline text-sm")}>
                {label}
            </Label>
...
export function SortArrow({ direction: direction }: { direction: string }) {
    return (
        <span className="text-violet-600 px-0.5 h-2 w-2">
            {direction === "asc" ? " ▲" : " ▼"}
        </span>
    );
}
